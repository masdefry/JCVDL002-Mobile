...
const Home = ({navigation: {navigate}}) => {
    ...
    const onFillData = (val, dataType) => {
        if(dataType === 'from') return setData({...data, from: val}) // to, totalSeat, date: currentValue, from: newValue
        if(dataType === 'to') return setData({...data, to: val}) // from, totalSeat, date: currentValue, to: newValue
        if(dataType === 'totalSeat'){
            if(val > 3) return setError('Total Seat Maksimal 3')
            return setData({...data, totalSeat: val})
        }
            ...
            <Content>
                ...
                <Grid style={{alignSelf: 'center', ...spacing.pxThree, ...spacing.pyZero}}>
                    <Row style={{...spacing.pxZero, ...spacing.pyFive}}>
                        <Button onPress={() => onSearch()} style={{width: '100%', borderRadius: 3, ...color.bgPrimary}} block>
